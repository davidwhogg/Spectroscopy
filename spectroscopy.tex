...
\fluxLSF_j \equiv
  ...
  \,\transpose{\Amatrix_j}\,\inverse{\pixelvariance_i}\,\pixels_i \quad ,
\end{equation}
and then smoothing the vectors $\fluxLSF_j$ to make new vectors that
have uncorrelated errors (diagonal covariance).  This methodology is
``optimal'' in that the flux vectors $\fluxLSF_j$ are the
maximum-likelihood flux vectors given the pixel data $\pixels_i$; the
smoothing is just a (bad) method for handling the ``ringing'' that
occurs because neighboring pixels tend to be strongly anti-correlated
in any simple point estimate of the flux.

[SAM: Do I have my $\pixelvariance_i$ matrices in the right places?---DWH]

[SAM: What I have written above and below \emph{must be wrong} because
  there is also sky $\sky_i$.  I need to fix that.---DWH]
...
uninformative; in this case the least-squares solution developed above
becomes the mean, and the least-squares covariance matrix becomes the
variance of the posterior, which is exactly a (high-dimensional)
normal distribution.  Note that the variance tensor $\covariance_{ij}$
will \emph{not} in general be diagonal, although if the data are good,
