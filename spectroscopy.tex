...
If everything is well sampled, we expect the matrices to be
``smooth''.  That is, we expect matrix rows and columns to be similar:
\begin{eqnarray}\displaystyle
...
relationships between them.  Our challenge is to write down all of
these prior expectations in a (possibly approximate) Gaussian form
such that when we perform the inference, we can still use all the
convenient and fast machinery of convex, linear optimization.

In the above, we have assumed that the matrices $\Amatrix_j$ are all
identical for all the exposures under consideration.  Of course we do
expect some variations in these matrices from exposure to exposure
(from, for example, flexure in the spectrographs and atmospheric
transparency) and from month to month (from, for example, detector
decay, fiber damage, and dust build-up on the mirrors).  We want to
permit variation in the matrices but not arbitrary variation.  That
suggests going hierarchical?  Similarly, the sky, flatfield lamps, and
arc lamps will vary over time but again, not arbitrarily.  Our
challenge is to discover the variability and then implement priors
that once again respect convexity and linearity.
